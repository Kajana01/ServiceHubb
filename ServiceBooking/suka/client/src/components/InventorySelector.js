...
import { FaBox, FaSearch, FaFilter, FaExclamationTriangle, FaCheckCircle, FaSignInAlt } from 'react-icons/fa';
...
const InventorySelector = ({ serviceCategory, onInventorySelect, selectedItems = [], onError }) => {
  ...
  const handleQuantityChange = (itemId, quantity) => {
    const updatedItems = selectedItems.map(item => 
      item._id === itemId 
        ? { ...item, selectedQuantity: Math.max(1, Math.min(quantity, item.quantity)) }
        : item
    );
  ...
  const totalCostBreakdown = selectedItems.map(item => ({
    name: item.name,
